...
class EEG_Processing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    final float SEIZURE_THRESHOLD = 1.5;
    boolean passedThreshold = false;
    float FFT_freq_Hz, FFT_value_uV;
    for (int Ichan=0; Ichan < nchan; Ichan++) {
      //loop over each new sample
      float avg_FFT_value_uV = avg_FFT(fftData[Ichan]); //Calculate sample avg
      for (int Ibin=0; Ibin < fftBuff[Ichan].specSize(); Ibin++) {
        FFT_freq_Hz = fftData[Ichan].indexToFreq(Ibin);
        FFT_value_uV = fftData[Ichan].getBand(Ibin);

        //add your processing here...
        if (FFT_freq_Hz >= 2.0 && FFT_freq_Hz < 4.0) {
          if (FFT_value_uV > SEIZURE_THRESHOLD * avg_FFT_value_uV && passedThreshold == false) {
    ...
    if (passedThreshold) {
      PostRequest post = new PostRequest("https://ellipsy.herokuapp.com/api/seizure");
      post.addData("data", Arrays.toString(data_forDisplay_uV));
      post.send();
      println("Reponse Content: " + post.getContent() + new Integer(new Random().nextInt()).toString());
    }
  }

  private float avg_FFT(FFT nums) {
    return nums.calcAvg(2.0, 4.0);
  }
